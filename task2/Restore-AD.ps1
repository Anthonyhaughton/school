...
# 1. Check if Finance OU exists
$financeExists = Get-ADOrganizationalUnit -Filter {Name -eq 'Finance'} -ErrorAction SilentlyContinue

if ($financeExists) {
    Write-Host "The 'Finance' OU exists. Removing.."
...
try {
    ...
    Write-Host "The 'AdResults.txt' file was created."
}
catch {
    Write-Host "An error occurred while creating file: $_"
}